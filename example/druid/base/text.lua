...
local Event = require("example.druid.event")
local const = require("example.druid.const")
local helper = require("example.druid.helper")
local utf8_lua = require("example.druid.system.utf8")
local component = require("example.druid.component")
...
local function update_text_area_size(self)
	...
	if self:is_multiline() then
		...
		--not original:
		---[[
		scale_modifier=math.min(self.start_scale.y, math.sqrt(max_height/metrics.height)^(1.5+2^-4))
		--]]
